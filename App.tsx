...
const STORAGE_KEY = 'brain_test_stable_v1';
  ...
  if (view === 'SPLASH') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-blue-600 text-white p-10 text-center">
        ...
        <h1 className="text-5xl font-black mb-2 tracking-tighter italic">Brain Test Lite</h1>
        <p className="text-blue-200 text-[10px] font-black uppercase tracking-[0.5em] mb-16 opacity-80">Stable Build v1.3</p>
      ...
      <div className="flex flex-col min-h-screen bg-[#FDFDFF] animate-in slide-in-from-bottom duration-500">
        ...
        <div className="flex-1 flex flex-col justify-center items-center px-10">
           <div className="w-full bg-white border border-gray-100 rounded-[3rem] p-12 shadow-2xl shadow-blue-100/10 mb-14 text-center">
              ...
              <div className="inline-flex items-center gap-2 px-4 py-2 bg-blue-50 text-blue-600 rounded-full text-[10px] font-black border border-blue-100"><Sparkles className="w-4 h-4" /> LEVEL {gameState.currentLevel}</div>
           </div>
           <div className="w-full max-w-xs space-y-4">
              <button onClick={() => setView('PLAY')} className="w-full py-6 bg-blue-600 text-white rounded-[2rem] font-black text-xl shadow-2xl flex items-center justify-center gap-4 active:scale-95 transition-all">Continue Quest</button>
      ...
      <div className="p-8 flex-1 flex flex-col max-w-md mx-auto w-full">
        ...
        <div className="fixed bottom-10 left-0 right-0 p-10 flex gap-4 max-w-md mx-auto">
            ...
            {feedback.show && feedback.type === 'correct' ? (
              <button onClick={() => { setFeedback(f => ({ ...f, show: false })); setSelectedIdx(null); setGameState(p => ({ ...p, currentLevel: p.currentLevel + 1 })); }} className="flex-[3] py-5 bg-blue-600 text-white rounded-[2rem] font-black shadow-xl active:scale-95 flex items-center justify-center gap-2">Next Level</button>
            ) : (
