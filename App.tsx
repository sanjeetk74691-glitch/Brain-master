...
const STORAGE_KEY = 'brain_test_v11_stable';
...
export default function App() {
  ...
  const handleKeyAuth = async () => {
    if (window.aistudio) {
      ...
      alert("API_KEY missing. Please set it in Vercel settings.");
    }
  };
  ...
  const generateAIQuest = async () => {
    ...
    try {
      ...
      const types = ['MCQ', 'IMAGE_MCQ', 'FILL_BLANKS'];
      const qType = types[Math.floor(Math.random() * types.length)];

      const logicRes = await ai.models.generateContent({
        model: 'gemini-3-pro-preview',
        contents: `Create a tricky and funny lateral thinking brain teaser. 
        Type: ${qType}. 
        Return strictly JSON format:
        {
          "type": "${qType}",
          "prompt": { "en": "Question text", "hi": "हिंदी में सवाल" },
          "options": { "en": ["Option A", "B", "C", "D"], "hi": ["विकल्प क", "ख", "ग", "घ"] },
          "answer": 0,
          "hint": { "en": "Helpful clue", "hi": "संकेत" }
        }`,
        config: {
      ...
      if (qType === 'IMAGE_MCQ') {
        const imgRes = await ai.models.generateContent({
          ...
          contents: { parts: [{ text: `A vibrant, high-contrast minimalist vector illustration of this riddle: ${data.prompt.en}` }] },
        });
        for (const part of imgRes.candidates?.[0]?.content?.parts || []) {
          if (part.inlineData) {
            newQ.imageUrl = `data:image/png;base64,${part.inlineData.data}`;
            break;
          }
        }
      }
      ...
      console.error(err);
      alert("Neural Lab busy. Using local matrix.");
      setView('PLAY');
    } finally {
  ...
  if (view === 'SPLASH') {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-blue-600 text-white p-10 text-center">
        ...
        <p className="text-blue-200 text-[10px] font-black uppercase tracking-[0.5em] mb-16 opacity-80">Neural Core v1.1</p>
    ...
    return (
      <div className="flex flex-col min-h-screen bg-[#FDFDFF] animate-in slide-in-from-bottom duration-500">
        ...
        <div className="flex-1 flex flex-col justify-center items-center px-10">
           <div className="w-full bg-white border border-gray-100 rounded-[4rem] p-12 shadow-2xl shadow-blue-100/10 mb-14 text-center relative overflow-hidden">
              <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mb-4">Current Rank</p>
           ...
           <div className="w-full max-w-xs space-y-4">
              ...
              <button onClick={() => setView('PLAY')} className="w-full py-5 bg-white border-2 border-gray-100 text-gray-700 rounded-[2rem] font-black text-lg flex items-center justify-center gap-4 active:scale-95 shadow-sm">Classic Levels</button>
              <div className="grid grid-cols-2 gap-4">
                 ...
                 <button onClick={() => setView('SETTINGS')} className="py-5 bg-white border border-gray-100 rounded-2xl flex flex-col items-center gap-2 active:scale-95 shadow-sm"><Settings className="w-6 h-6 text-gray-400" /><span className="text-[10px] font-black text-gray-400 uppercase">Setup</span></button>
              </div>
           </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col min-h-screen bg-gray-50 overflow-y-auto pb-40">
      <HUD title={view === 'AI_LAB' ? "Neural Synthesis" : `Level ${gameState.currentLevel}`} />
      
      {isGenerating ? (
        <div className="flex-1 flex flex-col items-center justify-center p-12 text-center">
          <div className="w-24 h-24 border-[6px] border-blue-100 border-t-blue-600 rounded-full animate-spin mb-8"></div>
          <h3 className="text-2xl font-black text-gray-800">Synthesizing Puzzle...</h3>
        </div>
      ) : (
        <div className="p-8 flex-1 flex flex-col max-w-md mx-auto w-full">
          <div className="bg-white p-10 rounded-[3.5rem] shadow-xl border border-gray-100 mb-8 flex flex-col items-center text-center">
              {currentQ.imageUrl && <div className="mb-8 w-full h-64 rounded-3xl overflow-hidden bg-gray-50 ring-4 ring-blue-50"><img src={currentQ.imageUrl} className="w-full h-full object-cover" alt="Puzzle Clue" /></div>}
              ...
              {currentQ.type === 'FILL_BLANKS' ? (
                ...
                (currentQ.options?.[gameState.language] || currentQ.options?.en || []).map((opt, idx) => (
                  <button 
                    ...
                    className={`p-6 rounded-[2rem] font-black text-xl text-left active:scale-95 shadow-lg border-2 transition-all ${
                      selectedIdx === idx 
                        ? (idx === currentQ.answer ? 'bg-green-500 border-green-600 text-white animate-bounce' : 'bg-red-500 border-red-600 text-white animate-shake')
          ...
          <div className="fixed bottom-10 left-0 right-0 p-10 flex gap-4 max-w-md mx-auto">
              ...
              {feedback.show && feedback.type === 'correct' ? (
                <button onClick={() => {
                  setFeedback(f => ({ ...f, show: false }));
                  setSelectedIdx(null);
                  if (view === 'AI_LAB') { generateAIQuest(); } 
                  else { setGameState(p => ({ ...p, currentLevel: p.currentLevel + 1 })); }
      ...
      {feedback.show && feedback.type === 'correct' && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center pointer-events-none">
           <div className="bg-white/80 backdrop-blur-md rounded-full px-12 py-6 shadow-2xl animate-in zoom-in duration-300 border-4 border-green-500">
              <h2 className="text-4xl font-black text-green-600 italic">GENIUS!</h2>
           </div>
        </div>
      )}
    </div>
